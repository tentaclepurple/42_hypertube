...
export default function Search() {
    ...
    }, [debouncedQuery, buildQueryString, logout, isLoggingOut, hasActiveFilters]);

    useEffect(() => {
        if (debouncedQuery || hasActiveFilters()) {
            fetchMovies(page);
        }
    }, [debouncedQuery, page, filters.genres, filters.yearFrom, filters.yearTo, fetchMovies, hasActiveFilters]);

    useEffect(() => {
        const observer = new IntersectionObserver(
            (entries) => {
                if (entries[0].isIntersecting && hasMore && !loading && !fetchingRef.current && (debouncedQuery || hasActiveFilters())) {
    ...
    }, [hasMore, debouncedQuery, loading, hasActiveFilters]);

    const shouldShowInitialMessage = !debouncedQuery && !hasActiveFilters() && !isDebouncing && !loading;
    const shouldShowLoading = (loading || isDebouncing) && filteredMovies.length === 0;
    const shouldShowNoResults = !loading && !isDebouncing && filteredMovies.length === 0 && (debouncedQuery || hasActiveFilters());
            ...
            <div className="min-h-[400px]">
                ...
                {shouldShowLoading && (
                    <div className="text-center py-10">
                        ...
                        <p className="mt-4 text-gray-400">{t("search.loading")}</p>
                    </div>
                )}
                {shouldShowNoResults && (
                    <div className="text-center py-10">
